...
public class ImageFrame3 {
    ...
    // Method to add double frame to the Image.
    public static BufferedImage framedImage(BufferedImage sourceImage, int frameSize, String colorOfUpperFrame, String colorOfLowerFrame) {
        ...
        for (int col = 0; col < framedImage.getWidth(); col++) {
            for (int row = 0; row < framedImage.getHeight(); row++) {
                if ((col <= ((frameSize / 2) - INDEX_OF_nth_COUNT) || col >= framedImage.getWidth() - (frameSize / 2)) || (row <= ((frameSize / 2) - INDEX_OF_nth_COUNT) || row >= framedImage.getHeight() - (frameSize / 2))) {
                    framedImage.setRGB(col, row, hexaDecimalValueOfColor(colorOfUpperFrame));
                    ...
                    framedImage.setRGB(col, row, hexaDecimalValueOfColor(colorOfLowerFrame));
                } else {
...
/*
BufferedImage sourceImage = ImageFrame3.readImageFromDisk("D:\\CAS\\Image Processing Pictures\\ImageInFrame\\wheeling.jpg");
        BufferedImage effectImage = ImageFrame3.framedImage(sourceImage, 400, "navy", "LIME");
        ImageFrame3.writeImageToDisk(effectImage, "jpg", "D:\\CAS\\Image Processing Pictures\\ImageInFrame\\wheeling2.jpg");
