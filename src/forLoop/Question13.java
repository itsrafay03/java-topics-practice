...
public class Question13 {
    ...
    public Question13(int valueOfx, int limit) {
    ...
    // Input Method.
    public void input(){
        System.out.print("The series is [ 1+x+x^2/2! +x^3/3! +....].\nEnter value of 'x' : ");
        setValueOfx(scanner.nextInt());
        System.out.print("Enter the number of terms of series you want : ");
        setLimit(scanner.nextInt());
    }

    // Method to display the sum of the series [ 1+x+x^2/2! +x^3/3! +....].
    public double printSumOfSeries(){
        double valueOfTerm;
        double sumOfTerm = 1 + getValueOfx();
        for (int term = 2; term < getLimit(); term++) {
            valueOfTerm = (double) power(getValueOfx(), term) / factorial(term);
...
/*
        ...
        question13.input();
        System.out.println(question13.printSumOfSeries());
 */